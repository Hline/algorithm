...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int m = 0; m < M; m++) {
			...
			bw.write(QueryMin(0, N - 1, 1, a, b) + " " + QueryMax(0, N - 1, 1, a, b) + "\n");
		}
		bw.close();
	...
	private static int InitMax(int start, int end, int now) {
		// 리프노드 (1~1)
		if (start == end)
			return treeMax[now] = input[start];
	...
	private static void UpdateMax(int start, int end, int now, int target, int diff) {
		// 탐색하는 트리범위 내에 값이 변경되는 노드(target)가 없을 경우 (범위 밖)
		// (1) target start~end (2) start~end target
		if (target < start || end < target)
			return;
		...
		int mid = (start + end) / 2;
		UpdateMax(start, mid, 2 * now, target, diff);
		UpdateMax(mid + 1, end, 2 * now + 1, target, diff);
	}
